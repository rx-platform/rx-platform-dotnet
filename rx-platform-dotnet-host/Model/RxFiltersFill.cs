...
namespace ENSACO.RxPlatform.Hosting.Model.Algorithms
{
    ...
    internal class RxFiltersFill : IRxMetaAlgorithm
    {
        private List<RxFilterDataItem>? GetItems(PropertyInfo[] properties, object instance)
        {
            var items = new List<RxFilterDataItem>();
            foreach (var prop in properties)
            {
                ...
                if (prop.SetMethod != null)
                {
                    ...
                    readOnly = !requiredModifiers.Contains(typeof(System.Runtime.CompilerServices.IsExternalInit));
                }
                if (propType == null)
                {
                    continue;
                }
                var attr = propType.GetCustomAttribute<RxPlatformFilterType>(false);
                if (attr != null)
                {
                    ...
                    {
                        RxFilterDataItem item = new RxFilterDataItem
                        {
                            name = prop.Name,
        ...
        private void FillTypes<T>(Dictionary<RxNodeId, PlatformTypeBuildMeta<T>> data) where T : RxPlatformTypeAttribute
        {
            foreach (var kvp in data)
            {
                ...
                var props = ReflectionHelpers.GetFilterPropertyInfos(objType.type);
